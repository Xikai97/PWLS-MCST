...
clear ; close all;
LayerNum = 2;
numBlock = [5,5];   % number of clusters in each layer [K_1,K_2,...,K_L]
PatchSize = 8;
...
% set the path of toolbox
addpath(genpath('../toolbox'));

% set the path of training dataset
load('./data_XCAT/phantom_crop154.mat');
patch=[];

for ii=[10,30,40,50,80]  % choose the training slices from dataset
% for ii = 45
...
for j = 1 : iter
    ...
    for l = 1:1:LayerNum
        CluMap{l} = ClusterMap(ImgSiz, PatSiz, SldDist, IDX{l}(:,end*4/5+1:end), PatNum, numBlock(l));
        R_image{l} = col2imstep(single(R{l+1}(:,size(patch,2)*4/5+1:end)),size(image),PatSiz)/prod(PatSiz);
    end
...
% Revise this line when you change the model depth
save(sprintf('./learned_trans_mcst_XCAT/XCAT_mcst_nodropmean_%g_%g_numBlock_%g_%g_eta_%giter_%gPatchSize.mat',...
